...
object Functions extends App {
  ...
  favoriteDonutT.foreach(d => println(s"Favorite donut = $d"))

  val leastFavoriteDonutT: Option[String] = None
  leastFavoriteDonutT.foreach(d => println(s"Least Favorite donut = $d"))
  ...
  dailyCouponCode().foreach(couponCode => println(s"Today's coupon code = $couponCode"))
