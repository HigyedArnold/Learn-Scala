...
object CollectionFunctionsNZ extends App {
  ...
  val resMin =
    //noinspection SimplifiableFoldOrReduce
    donutPrices.reduce((x: Double, y: Double) => x min y)
  println(s"Cheapest donut price = $resMin")

  println("\nStep 5: How to find the most expensive donut using reduce function")

  val resMax =
    //noinspection SimplifiableFoldOrReduce
    donutPrices.reduce((x: Double, y: Double) => x max y)
  println(s"Most expensive donut price = $resMax")
