package com.aibuild.fpinscala.datastructures
...
object Tree {
  ...
  def main(args: Array[String]): Unit = {
    val tree: Tree[Int] = Branch(Branch(Leaf(1), Branch(Leaf(2), Leaf(3))), Branch(Branch(Leaf(4), Leaf(5)), Leaf(6)))

    println("Tree: " + tree.toString)
    println("Size: " + size(tree))
    println("Maximum: " + maximum(tree))
    println("Depth: " + depth(tree))
    println("SizeF: " + sizeViaFold(tree))
    println("MaximumF: " + maximumViaFold(tree))
    println("DepthF: " + depthViaFold(tree))
